...
def predict(request, ticker_value, number_of_days):
    ...
    try:
        df_ml = yf.download(tickers = ticker_value, start='2010-01-01')
    except:
        tickers_value = 'AAPL',
        df_ml = yf.download(tickers = tickers_value, start='2010-01-01')
    ...
    forecast_df.to_csv('app/Data/forecast_data.csv', index=False)

    # Load the forecast data from the CSV file
    forecast_df = pd.read_csv('app/Data/forecast_data.csv')

     # Load the sentiment data from the CSV file
    sentiment_df = pd.read_csv('app/Data/sentiment.csv')
    filtered_sentiment_df = sentiment_df[sentiment_df['Positive'] > 40]

    # Increase all forecasted values by 10%
    forecast_df['Forecast'] = forecast_df['Forecast'] * 1.1
    forecast_df.to_csv('app/Data/increased_forecast_data.csv', index=False)

    #==========================================PLOTTING=====================================
    pred_dict = {"Date": [], "Prediction": []}

    for i in range(len(forecast_df)):
        forecast_value = forecast_df.loc[i, 'Forecast']
    
        if i < len(filtered_sentiment_df):
            prediction = forecast_value  # Keep the increased value

        else:
            prediction = forecast_value # Keep the original value
    
        pred_dict["Date"].append(dt.datetime.today() + dt.timedelta(days=i))
        pred_dict["Prediction"].append(prediction)
    
    pred_df = pd.DataFrame(pred_dict)
    last_day_value = pred_df['Prediction'].iloc[-1]
    
    pred_fig = go.Figure([go.Scatter(x=pred_df['Date'], y=pred_df['Prediction'], line=dict(color='gold'))])
    pred_fig.update_xaxes(rangeslider_visible=True)
    pred_fig.update_layout(
        paper_bgcolor="#14151b",
        plot_bgcolor="#14151b",
        font_color="white",
        xaxis_title=f"Predicted Value for {len(forecast_df)} Days ({last_day_value})",
    ...
    plot_div_pie = plot(fig, auto_open=False, output_type='div')
    ...
    plot_div_news = plot(fig, auto_open=False, output_type='div')
